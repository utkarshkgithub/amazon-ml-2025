...
def train_xgboost_log(X_train, X_val, y_train, y_val, y_train_log, y_val_log, X_test):
    ...
    print("📋 Model Configuration (LOG-SCALE OPTIMIZED):")
    ...
    if val_r2 > 0.70:
        ...
        print(f"   Validation R²: ❌ POOR ({val_r2:.3f}) - Needs work")
    ...
    print(f"\n🔍 Sanity Checks:")
    print(f"   Negative prices: {negative_count} {'✅' if negative_count == 0 else '❌'}")
...
if __name__ == "__main__":
    ...
    print("   models/xgboost_log_predictions.csv  ← SUBMIT THIS!")
    ...
    if r2 > 0.70:
        ...
        print("   ⚠️  R² > 0.40: Fair, but try ensemble next")
    else:
        print("   ❌ R² < 0.40: Something's wrong, check data")
