...
def train_xgboost(X_train, X_val, y_train, y_val, X_test):
    """Train XGBoost with progress tracking and early stopping"""
    ...
    # Train with simplified fit call
    model.fit(
        X_train, y_train,
    ...
    pred_path = 'models/xgboost_predictions.csv'
    ...
    imp_path = 'models/xgboost_feature_importance.csv'
    ...
    summary_path = 'models/xgboost_summary.csv'
...
if __name__ == "__main__":
    print("\n" + "="*70)
    print("XGBOOST PRICE PREDICTION TRAINING")
    print("="*70)
    print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    # Train
    model, mae, rmse, r2 = train_xgboost(X_train, X_val, y_train, y_val, X_test)
    ...
    print("   models/xgboost_predictions.csv  ← Submit this!")
    print("   models/xgboost_feature_importance.csv")
    print("   models/xgboost_summary.csv\n")
    
    print("🎯 Next steps:")
    print("   1. Check feature importance to see image vs text balance")
    print("   2. Train LightGBM for comparison")
    print("   3. Hyperparameter tuning if needed\n")
